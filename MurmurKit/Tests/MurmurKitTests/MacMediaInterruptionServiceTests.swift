...
func mediaKeyTapLocationDefaultsToHID() {
    let tap = murmurMediaKeyTapLocation(environment: [:])
...
func mediaKeyTapLocationHonorsAnnotatedOverride() {
    let tap = murmurMediaKeyTapLocation(environment: ["MURMUR_MEDIA_KEY_TAP": "annotated"])
...
func mediaKeyTapLocationFallsBackToHIDForInvalidOverride() {
    let tap = murmurMediaKeyTapLocation(environment: ["MURMUR_MEDIA_KEY_TAP": "nope"])
    #expect(tap == .cghidEventTap)
}
